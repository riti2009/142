...
def recommended_articles():
    ...
    all_reccommended = pd.DataFrame(columns=col_names)
    for article in liked_articles:
        output = get_recommendations(article['contentId'])
        all_reccommended = all_reccommended.append(output)
    all_reccommended.drop_duplicates(subset = ['title'],inplace = True)
    recommended_data = []
    for i,r in all_reccommended.iterrows():
        d = {
            'url':r['url'],
            'title':r['title'],
            'text':r['text'],
